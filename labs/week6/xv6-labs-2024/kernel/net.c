...
static struct bound_port *find_bound_port(uint16 port) {
...
uint64 sys_recv(void) {
    ...
    if (copyout(p->pagetable, src_arg, (char *)&pp->src_ip,
                sizeof(pp->src_ip)) < 0 ||
        copyout(p->pagetable, sport_arg, (char *)&pp->src_port,
                sizeof(pp->src_port)) < 0) {
        goto bad;
    }
