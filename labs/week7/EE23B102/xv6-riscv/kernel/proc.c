...
kfork(void)
{
  ...
  np->sz = p->sz;
  np->cur_vma_start = p->cur_vma_start;
...
kexit(int status)
{
  ...
  for(int i = 0; i < NVMA; i++){
    ...
    v->valid = 0;
    pte_t *pte;
    for(uint64 j = v->start; j <= v->end; j += PGSIZE){
      pte = walk(p->pagetable, j, 0);
      if(pte && (*pte & PTE_V)){ // separately handle mmap shared files
        if((v->flags & MAP_SHARED) && v->file){
          uint64 file_off = v->offset + (j - v->start);
          struct file *f = v->file;
          shared_decref(f->ip, file_off);
