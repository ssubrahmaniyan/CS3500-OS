...
malloc(uint nbytes)
{
 ...
 a32:	ec5ff0ef          	jal	ra,8f6 <free>
  return freep;
 a36:	00093503          	ld	a0,0(s2)
      if((p = morecore(nunits)) == 0)
 a3a:	dd61                	beqz	a0,a12 <malloc+0x94>
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 a3c:	611c                	ld	a5,0(a0)
    if(p->s.size >= nunits){
 a3e:	4798                	lw	a4,8(a5)
 a40:	fa9777e3          	bgeu	a4,s1,9ee <malloc+0x70>
    if(p == freep)
 a44:	00093703          	ld	a4,0(s2)
 a48:	853e                	mv	a0,a5
 a4a:	fef719e3          	bne	a4,a5,a3c <malloc+0xbe>
  p = sbrk(nu * sizeof(Header));
 a4e:	8552                	mv	a0,s4
 a50:	af3ff0ef          	jal	ra,542 <sbrk>
  if(p == (char*)-1)
 a54:	fd551ce3          	bne	a0,s5,a2c <malloc+0xae>
        return 0;
 a58:	4501                	li	a0,0
 a5a:	bf65                	j	a12 <malloc+0x94>
