...
{
 348:	715d                	addi	sp,sp,-80
 34a:	e486                	sd	ra,72(sp)
 34c:	e0a2                	sd	s0,64(sp)
 34e:	fc26                	sd	s1,56(sp)
 350:	f84a                	sd	s2,48(sp)
 352:	f44e                	sd	s3,40(sp)
 ...
 3d8:	60a6                	ld	ra,72(sp)
 3da:	6406                	ld	s0,64(sp)
 3dc:	74e2                	ld	s1,56(sp)
 3de:	7942                	ld	s2,48(sp)
 3e0:	79a2                	ld	s3,40(sp)
 3e2:	6161                	addi	sp,sp,80
 3e4:	8082                	ret
    x = -xx;
...
malloc(uint nbytes)
{
 ...
 802:	ec5ff0ef          	jal	ra,6c6 <free>
  return freep;
 806:	00093503          	ld	a0,0(s2)
      if((p = morecore(nunits)) == 0)
 80a:	dd61                	beqz	a0,7e2 <malloc+0x94>
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 80c:	611c                	ld	a5,0(a0)
    if(p->s.size >= nunits){
 80e:	4798                	lw	a4,8(a5)
 810:	fa9777e3          	bgeu	a4,s1,7be <malloc+0x70>
    if(p == freep)
 814:	00093703          	ld	a4,0(s2)
 818:	853e                	mv	a0,a5
 81a:	fef719e3          	bne	a4,a5,80c <malloc+0xbe>
  p = sbrk(nu * sizeof(Header));
 81e:	8552                	mv	a0,s4
 820:	af3ff0ef          	jal	ra,312 <sbrk>
  if(p == (char*)-1)
 824:	fd551ce3          	bne	a0,s5,7fc <malloc+0xae>
        return 0;
 828:	4501                	li	a0,0
 82a:	bf65                	j	7e2 <malloc+0x94>
