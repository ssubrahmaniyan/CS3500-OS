...
malloc(uint nbytes)
{
 ...
 854:	ec5ff0ef          	jal	ra,718 <free>
  return freep;
 858:	00093503          	ld	a0,0(s2)
      if((p = morecore(nunits)) == 0)
 85c:	dd61                	beqz	a0,834 <malloc+0x94>
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 85e:	611c                	ld	a5,0(a0)
    if(p->s.size >= nunits){
 860:	4798                	lw	a4,8(a5)
 862:	fa9777e3          	bgeu	a4,s1,810 <malloc+0x70>
    if(p == freep)
 866:	00093703          	ld	a4,0(s2)
 86a:	853e                	mv	a0,a5
 86c:	fef719e3          	bne	a4,a5,85e <malloc+0xbe>
  p = sbrk(nu * sizeof(Header));
 870:	8552                	mv	a0,s4
 872:	af3ff0ef          	jal	ra,364 <sbrk>
  if(p == (char*)-1)
 876:	fd551ce3          	bne	a0,s5,84e <malloc+0xae>
        return 0;
 87a:	4501                	li	a0,0
 87c:	bf65                	j	834 <malloc+0x94>
