...
{
     ca8:	715d                	addi	sp,sp,-80
     caa:	e486                	sd	ra,72(sp)
     cac:	e0a2                	sd	s0,64(sp)
     cae:	fc26                	sd	s1,56(sp)
  ...
  while(--i >= 0)
     ...
     d3e:	7942                	ld	s2,48(sp)
     d40:	79a2                	ld	s3,40(sp)
    ...
    1062:	00e6ea63          	bltu	a3,a4,1076 <free+0x50>
{
  ...
  if(bp + bp->s.size == p->s.ptr){
    ...
    107c:	02059813          	slli	a6,a1,0x20
    1080:	01c85713          	srli	a4,a6,0x1c
  ...
  if(p + p->s.size == bp){
    ...
    1090:	02061593          	slli	a1,a2,0x20
    1094:	01c5d713          	srli	a4,a1,0x1c
...
malloc(uint nbytes)
{
  ...
  if(p == (char*)-1)
    ...
    111c:	b7e1                	j	10e4 <malloc+0x36>
      if(p->s.size == nunits)
    111e:	02e48c63          	beq	s1,a4,1156 <malloc+0xa8>
        p->s.size -= nunits;
    1122:	4137073b          	subw	a4,a4,s3
    1126:	c798                	sw	a4,8(a5)
        p += p->s.size;
    1128:	02071693          	slli	a3,a4,0x20
    112c:	01c6d713          	srli	a4,a3,0x1c
    ...
    115a:	bff1                	j	1136 <malloc+0x88>
  hp->s.size = nu;
    115c:	01652423          	sw	s6,8(a0)
  free((void*)(hp + 1));
    1160:	0541                	addi	a0,a0,16
    1162:	ec5ff0ef          	jal	ra,1026 <free>
  return freep;
    1166:	00093503          	ld	a0,0(s2)
      if((p = morecore(nunits)) == 0)
    116a:	dd61                	beqz	a0,1142 <malloc+0x94>
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    116c:	611c                	ld	a5,0(a0)
    if(p->s.size >= nunits){
    116e:	4798                	lw	a4,8(a5)
    1170:	fa9777e3          	bgeu	a4,s1,111e <malloc+0x70>
    if(p == freep)
    1174:	00093703          	ld	a4,0(s2)
    1178:	853e                	mv	a0,a5
    117a:	fef719e3          	bne	a4,a5,116c <malloc+0xbe>
  p = sbrk(nu * sizeof(Header));
    117e:	8552                	mv	a0,s4
    1180:	af3ff0ef          	jal	ra,c72 <sbrk>
  if(p == (char*)-1)
    1184:	fd551ce3          	bne	a0,s5,115c <malloc+0xae>
        return 0;
    1188:	4501                	li	a0,0
    118a:	bf65                	j	1142 <malloc+0x94>
