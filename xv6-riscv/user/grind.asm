...
printint(int fd, long long xx, int base, int sgn)
{
     ...
     c62:	f84a                	sd	s2,48(sp)
     c64:	f44e                	sd	s3,40(sp)
  ...
  while(--i >= 0)
     ...
     cee:	74e2                	ld	s1,56(sp)
     cf0:	7942                	ld	s2,48(sp)
     cf2:	79a2                	ld	s3,40(sp)
     cf4:	6161                	addi	sp,sp,80
     cf6:	8082                	ret
    x = -xx;
...
malloc(uint nbytes)
{
  ...
  if(p == (char*)-1)
    ...
    10ce:	b7e1                	j	1096 <malloc+0x36>
      if(p->s.size == nunits)
    10d0:	02e48c63          	beq	s1,a4,1108 <malloc+0xa8>
        p->s.size -= nunits;
    10d4:	4137073b          	subw	a4,a4,s3
    10d8:	c798                	sw	a4,8(a5)
        p += p->s.size;
    10da:	02071693          	slli	a3,a4,0x20
    10de:	01c6d713          	srli	a4,a3,0x1c
    10e2:	97ba                	add	a5,a5,a4
        p->s.size = nunits;
    10e4:	0137a423          	sw	s3,8(a5)
      freep = prevp;
    10e8:	00001717          	auipc	a4,0x1
    10ec:	f2a73423          	sd	a0,-216(a4) # 2010 <freep>
      return (void*)(p + 1);
    10f0:	01078513          	addi	a0,a5,16
    ...
    110a:	e118                	sd	a4,0(a0)
    110c:	bff1                	j	10e8 <malloc+0x88>
  hp->s.size = nu;
    110e:	01652423          	sw	s6,8(a0)
  free((void*)(hp + 1));
    1112:	0541                	addi	a0,a0,16
    1114:	ec5ff0ef          	jal	ra,fd8 <free>
  return freep;
    1118:	00093503          	ld	a0,0(s2)
      if((p = morecore(nunits)) == 0)
    111c:	dd61                	beqz	a0,10f4 <malloc+0x94>
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    111e:	611c                	ld	a5,0(a0)
    if(p->s.size >= nunits){
    1120:	4798                	lw	a4,8(a5)
    1122:	fa9777e3          	bgeu	a4,s1,10d0 <malloc+0x70>
    if(p == freep)
    1126:	00093703          	ld	a4,0(s2)
    112a:	853e                	mv	a0,a5
    112c:	fef719e3          	bne	a4,a5,111e <malloc+0xbe>
  p = sbrk(nu * sizeof(Header));
    1130:	8552                	mv	a0,s4
    1132:	af3ff0ef          	jal	ra,c24 <sbrk>
  if(p == (char*)-1)
    1136:	fd551ce3          	bne	a0,s5,110e <malloc+0xae>
        return 0;
    113a:	4501                	li	a0,0
    113c:	bf65                	j	10f4 <malloc+0x94>
