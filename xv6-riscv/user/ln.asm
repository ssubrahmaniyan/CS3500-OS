...
malloc(uint nbytes)
{
 ...
 830:	ec5ff0ef          	jal	ra,6f4 <free>
  return freep;
 834:	00093503          	ld	a0,0(s2)
      if((p = morecore(nunits)) == 0)
 838:	dd61                	beqz	a0,810 <malloc+0x94>
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 83a:	611c                	ld	a5,0(a0)
    if(p->s.size >= nunits){
 83c:	4798                	lw	a4,8(a5)
 83e:	fa9777e3          	bgeu	a4,s1,7ec <malloc+0x70>
    if(p == freep)
 842:	00093703          	ld	a4,0(s2)
 846:	853e                	mv	a0,a5
 848:	fef719e3          	bne	a4,a5,83a <malloc+0xbe>
  p = sbrk(nu * sizeof(Header));
 84c:	8552                	mv	a0,s4
 84e:	af3ff0ef          	jal	ra,340 <sbrk>
  if(p == (char*)-1)
 852:	fd551ce3          	bne	a0,s5,82a <malloc+0xae>
        return 0;
 856:	4501                	li	a0,0
 858:	bf65                	j	810 <malloc+0x94>
