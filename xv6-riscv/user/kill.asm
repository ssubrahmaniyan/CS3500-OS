...
malloc(uint nbytes)
{
 ...
 834:	ec5ff0ef          	jal	ra,6f8 <free>
  return freep;
 838:	00093503          	ld	a0,0(s2)
      if((p = morecore(nunits)) == 0)
 83c:	dd61                	beqz	a0,814 <malloc+0x94>
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 83e:	611c                	ld	a5,0(a0)
    if(p->s.size >= nunits){
 840:	4798                	lw	a4,8(a5)
 842:	fa9777e3          	bgeu	a4,s1,7f0 <malloc+0x70>
    if(p == freep)
 846:	00093703          	ld	a4,0(s2)
 84a:	853e                	mv	a0,a5
 84c:	fef719e3          	bne	a4,a5,83e <malloc+0xbe>
  p = sbrk(nu * sizeof(Header));
 850:	8552                	mv	a0,s4
 852:	af3ff0ef          	jal	ra,344 <sbrk>
  if(p == (char*)-1)
 856:	fd551ce3          	bne	a0,s5,82e <malloc+0xae>
        return 0;
 85a:	4501                	li	a0,0
 85c:	bf65                	j	814 <malloc+0x94>
